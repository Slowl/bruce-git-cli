...
const { r, b, f, p } = minimist(process.argv.slice(2))
...
switch (action) {
	...
	case ACTION.COMMIT: {
		await createCommit({ r, p }).then(({ gitCommitMessage, currentBranch }) => {
			information.git_commit_create_success()
			information.git_commit_message(gitCommitMessage)
			p ? information.git_branch_push_success({ branchToPush: currentBranch}) : information.reminder_commit_push()
		})
		break
	}
	case ACTION.REBASE: {
